// app/users/[username]/following/page.tsx
"use client";
...
export default function FollowingListPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // following: follows.follower_id = owner_id -> followee_id -> profiles
      const fRes = await supabase
        .from("follows")
        .select("followee_id")
        .eq("follower_id", ownerProf.user_id);
      ...
      const ids = (fRes.data ?? []).map((r: any) => r.followee_id) as string[];
      if (ids.length === 0) {
        setList([]);
        setLoading(false);
        return;
  ...
  return (
    <Box sx={{ maxWidth: 960, mx: "auto", px: { xs: 1.5, sm: 2 }, py: 3 }}>
      ...
      <Breadcrumbs sx={{ mb: 1.5 }}>
        ...
        <Typography color="text.primary">フォロー中</Typography>
      </Breadcrumbs>
      ...
      >
        ...
        <Divider sx={{ my: 2 }} />
        <Chip label={`フォロー中 ${list.length} 人`} size="small" />
      </Paper>
